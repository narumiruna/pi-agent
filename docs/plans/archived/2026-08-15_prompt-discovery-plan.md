...
- Actionable P2, [include change time in mutation identities](https://github.com/narumiruna/pi-agent/pull/47#discussion_r3790800333): addressed by capturing `ctimeMs` and validating it again immediately before update or deletion renames; post-rename checks intentionally use stable identity fields because POSIX rename changes ctime, and same-size in-place update/delete regressions cover the failure mode.
- Actionable P2, [do not block snapshot reads on active runs](https://github.com/narumiruna/pi-agent/pull/47#discussion_r3790800334): addressed with coordinator snapshot readers that can run during chat or heartbeat, while pending and active maintenance retain exclusive priority; ordering regressions cover both paths.
- Actionable P1, [apply explicit trust during the committing reload](https://github.com/narumiruna/pi-agent/pull/47#discussion_r3790820313): addressed with a bounded resolution override set before native session reload, switched to the previous effective state for rollback, and cleared in `finally`; policy and service ordering regressions cover success, idempotence, reload failure, and persistence failure.
- Actionable P2, [evaluate trust handlers once per discovery boundary](https://github.com/narumiruna/pi-agent/pull/47#discussion_r3790833882): addressed by carrying the policy result through live and rollback reloads with the same bounded override; a filesystem marker integration regression proves the first-project handler executes once, and remembered decisions survive the override window.
- Evidence: the focused trust suites passed all 84 tests; PostgreSQL-enabled `npm run ci` passed all 443 tests; a transient conversation-fork polling failure passed three consecutive focused retries and the subsequent full Playwright run passed all 30 tests; `docker build -t pi-agent:local .` passed with image `sha256:cf94900ec035a938060ac9a8c6cd24655846280dd45bc5b2aed38338e032998a`; `git diff --check` passed.
