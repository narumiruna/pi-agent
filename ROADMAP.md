...
- [x] 未授權路徑、symlink escape、oversized body、stale write 與 active-run conflict 都有 server tests。
- [x] Navigation reload 後會回到有效頁面，失效 route 會安全返回 `Chats`。

## Phase 1：Chats

- [x] 將目前「對話」重新命名為 `Chats`，並保留現有 conversation list 與 new-chat 操作。
- [x] 沿用 `SessionManager` 與目前的 opaque conversation ID，不複製 JSONL session state。
